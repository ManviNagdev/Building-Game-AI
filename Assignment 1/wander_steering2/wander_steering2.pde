...
void draw() {
  ...
  randomOrient = randomBinomial();
  targetOrient = ch.orientation + randomOrient;
  desired_velocity = direction;
  target.x = ch.position.x + cos(randomOrient);
  target.y = ch.position.y + sin(randomOrient);
  target.x += 10*cos(targetOrient);
  target.y += 10*sin(targetOrient);
